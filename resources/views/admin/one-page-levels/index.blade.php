@extends('admin.layouts.admin',['title'=>'Manage One Page Levels'])
@section('title', 'Manage One Page Levels')
@section('content')

    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">{{ __('labels.manage_one_page_levels')}}</h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('labels.dashboard')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __('labels.manage_one_page_levels')}}</li>
                </ol>
            </nav>
        </div>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <a class="btn btn-inverse-info btn-rounded btn-icon add_data"
                           href="{{ url('admin/one-page-levels/create') }}" data-toggle="tooltip" data-placement="top"
                           data-original-title="Add One Page Level" title="Add One Page Level">
                        ...
                        <table class="table" id="one-page-level-table">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>{{ __('labels.confirm_delete_one_page_level')}}</p>
                </div>
                <div class="modal-footer">
                    <button type="button" value="{{URL::to('admin/one-page-levels')}}" class="btn btn-gradient-primary btn-rounded btn-fw yes_delete">{{ __('labels.submit')}}</button>
    ...
    @push('scripts')
        ...
        <script>
            var oTable = $('#one-page-level-table').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                ...
                ajax: {
                    url: '{{ url('admin/one-page-levels/get') }}',
                    type: 'POST',
            ...
            $(document).on("click", ".deleteOnePageLevel", function () {
                var one_page_level_id = $(this).attr('id');
                $(".yes_delete").attr('id', one_page_level_id);
                $("#exampleModalPrimary").modal('show');
            });

            $(document).on("click", ".yes_delete", function () {
                let one_page_level_id = $(this).attr('id');
                if(one_page_level_id){
                    $.ajax({
                        url: '{{URL::to('admin/one-page-levels')}}' + '/' + one_page_level_id,
                        type: 'DELETE',
                        headers: {
                            'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')
                        },
                        success: function (result) {
                            window.location = '{{URL::to('admin/one-page-levels')}}';
                        }
                    });
                }
            });
