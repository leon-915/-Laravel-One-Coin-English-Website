	...
	<div class="card-header" id="heading-1">
		<h5 class="mb-0">
			<a role="button" class="collapsed" data-toggle="collapse" href="#collapse-1" aria-expanded="false" aria-controls="collapse-1">
				{{__('labels.progress_level_and_rating_description')}}
            </a>
			</a>
		</h5>
	</div>
			...
			<div class="row">
                <div class="col-12">
                    <div class="plan_header">
                        <h2>{{__('labels.level_description')}}</h2>
                    </div>
                </div>
            </div>
				...
				@foreach ($levels as $key => $level)
					<div class="card">
						<div class="card-header" id="level-header-{{$level->id}}">
							<h5 class="mb-0">
								@if(isset($student->student_level_id) && $student->student_level_id == $level->id)
									<a class="collapsed" role="button"
									data-toggle="collapse" href="#onepage-levels-collapse-{{$level->id}}"
									aria-expanded="false"
									data-s_l_id="{{ $student->student_level_id }}"
